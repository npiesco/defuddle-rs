...
const STORAGE_KEY = "defuddle_rs_browser_state_v1";
  ...
  <body>
    ...
    <main>
      <article>
        ...
        <p>The best browser tools stay honest about what static hosting can and cannot do. CORS limits cross-origin fetches, so the product needs both direct URL reads and source-based fallback paths such as file import and pasted HTML.</p>
        <p>This sample exists so the app can be exercised end to end even before you point it at a real document.</p>
      </article>
...
function importExtensionCapture(payload) {
  if (!payload || typeof payload.html !== "string") {
    throw new Error("Extension capture payload was missing HTML.");
  }

  state.importedFileName = payload.title || payload.url || "Captured tab";
  state.importedFileContent = payload.html;
  state.htmlDraft = payload.html;
  state.htmlUrl = payload.url || "";
  state.fetchUrl = payload.url || "";
  syncInputsFromState();
  setMode("html");
  parseHtml(payload.html, payload.url || "https://example.com/article");
  setBanner(`Imported ${payload.url || "captured tab"} from the browser extension.`, "success");
}
...
function parseHtml(html, url) {
  ...
  if (!trimmed) {
    throw new Error("No HTML was provided. Paste HTML, import a file, or fetch a page first.");
  }
...
async function fetchPageHtml(url) {
  try {
    ...
    if (message.includes("Failed to fetch") || message.includes("NetworkError")) {
      throw new Error(
        `The browser could not read ${url} because the target does not allow cross-origin fetches from this origin. This app stays fully in-browser and does not proxy around CORS. Use HTML mode or File mode instead.`,
      );
    }
...
function wireEvents() {
  window.addEventListener("message", (event) => {
    ...
    if (!data || data.source !== "defuddle-rs-extension" || data.type !== "DEFUDDLE_IMPORT_CAPTURE") {
      return;
    }

    if (!state.wasmReady) {
      state.pendingExtensionCapture = data.payload;
      setBanner("Received a tab capture from the extension. Waiting for the WASM parser to finish loading…", "info");
      return;
    }

    try {
      importExtensionCapture(data.payload);
    } catch (error) {
      resetResult();
...
async function boot() {
  ...
  if (state.pendingExtensionCapture) {
    try {
      importExtensionCapture(state.pendingExtensionCapture);
      state.pendingExtensionCapture = null;
    } catch (error) {
      resetResult();
      setBanner(error.message, "error");
    }
