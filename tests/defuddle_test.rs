...
fn rust_blog_markdown_parity_with_defuddle() {
    ...
    let result =
        Defuddle::parse(&html, "https://blog.rust-lang.org/2024/02/08/Rust-1.76.0/").unwrap();
    assert_parity("rust_blog", &result.content_markdown, &expected, 1.0);
}
...
fn mdn_table_parity() {
    ...
    .unwrap();
    assert_parity("mdn_table", &result.content_markdown, &expected, 1.0);
}
...
fn hackernews_parity() {
    ...
    let result = Defuddle::parse(&html, "https://news.ycombinator.com/item?id=39232976").unwrap();
    assert_parity("hackernews", &result.content_markdown, &expected, 1.0);
}
...
fn fasterthanlime_parity() {
    ...
    .unwrap();
    assert_parity("fasterthanlime", &result.content_markdown, &expected, 1.0);
}
...
fn wikipedia_parity() {
    ...
    .unwrap();
    assert_parity("wikipedia", &result.content_markdown, &expected, 1.0);
}
...
fn github_tokio_parity() {
    ...
    let result = Defuddle::parse(&html, "https://github.com/tokio-rs/tokio").unwrap();
    assert_parity("github_tokio", &result.content_markdown, &expected, 1.0);
}
...
fn joel_test_parity() {
    ...
    .unwrap();
    assert_parity("joel_test", &result.content_markdown, &expected, 1.0);
}
